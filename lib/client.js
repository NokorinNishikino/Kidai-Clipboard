	...
	factory: (require) => {
		...
		function WindowHeader({ t, ctx, collapsed }) {
			...
			const onPointerDown = (e) => {
				...
				drag.current = { startX, startY, baseX, baseY, moved: false, id: e.pointerId, lastX: baseX, lastY: baseY, lastDock: null };
				const cancelFrame = () => {
				...
				const onMove = (ev) => {
					...
					d.lastY = y;
					d.lastDock = dock;
					// 实时位置写进 liveResize（这样任何一次重渲染都不会把面板弹回旧坐标），
					// 同时直接改 DOM 跟随指针；store 一律等松手再写。
					liveResize.x = x;
					liveResize.y = y;
					schedule();
				};
				const onUp = () => {
					...
					hideShield();
					// 松手才落库一次（含吸附判定结果）
					if (d !== null && d.moved === true) commitWindow(d.lastX, d.lastY, d.lastDock);
		...
		/** 拖动期的轻量挤出：复用缓存的滚动区元素，避免每帧重新 measureDock。 */
		function applyDockInsetWidth(width) {
			try {
				let el = liveResize.insetEl;
				if (el === null || el === undefined || el.isConnected !== true) {
					const geom = measureDock();
					el = geom !== null ? (geom.insetEl !== null && geom.insetEl !== undefined ? geom.insetEl : geom.rootEl) : null;
		...
		function applyDockInset(dock, width) {
			try {
				const geom = measureDock();
				if (geom === null) return () => {};
				const el = geom.insetEl !== null && geom.insetEl !== undefined ? geom.insetEl : geom.rootEl;
				if (el === null || el === undefined) return () => {};
				liveResize.insetEl = el;
		...
		function ResizeHandle({ t, data, docked }) {
			...
			const onPointerDown = (e) => {
				...
				liveResize.rootEl = rootEl;
				if (rootEl !== null && rootEl !== undefined) rootEl.classList.add("kidc_resizing");
				const hideShield = showDragShield(docked === true ? "ew-resize" : "nwse-resize");
				...
				active.current = true;
				dragRef.current = { startX: e.clientX, startY: e.clientY, baseW, baseH, lastW: baseW, lastH: baseH };
				...
				const onMove = (ev) => {
					...
					d.lastW = Math.round(Math.max(280, Math.min(d.baseW + delta, vw - 16)));
					if (docked !== true) {
