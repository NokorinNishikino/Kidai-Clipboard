...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			settingsMaxRecordsDesc: "保存会话快照时最多抓取的历史事件数",
		...
		const en = {
			...
			settingsMaxRecordsDesc: "Maximum history events captured per session snapshot",
		...
		function slimRecord(record) {
			...
			switch (type) {
				...
				case "assistant/message":
					return { ...base, role: "assistant", content: slimContent(data.message?.content), reason: typeof data.message?.source === "string" ? data.message.source : undefined };
				case "tool/call":
		...
		/** Capture the current session into a session snapshot entry. */
		async function captureSession(ctx, t) {
			...
			try {
				...
				const history = await fetchSessionHistory(ctx, currentId, undefined, maxRecords);
				const proj = history.projections ?? {};
				const title = proj.title !== null && proj.title !== undefined && proj.title !== ""
					? clip(String(proj.title), 120)
					: `会话 ${firstLine((history.header?.cwd ?? "") + "") || currentId.slice(0, 8)}`;
				const transcript = renderTranscript(history.records, history.messageCount, {
				...
				commitData((d) => {
					d.entries.unshift({
						...
						session: {
							...
							cwd: typeof history.header?.cwd === "string" ? history.header.cwd : null,
				...
				notifyMeta(history.messageCount > 0 ? "captured" : "capturedEmpty", { n: history.messageCount });
				return true;
			} catch (error) {
		...
		async function continueSession(ctx, t, entry) {
			...
			try {
				// 带分支点 fork；若分支点不可用（轮次未完成/会话状态变化），
				// 自动降级为不传 atSeq（DSH 语义 = 从最后一个已完成轮次分支）。
				let lastError = null;
				const attempts = [
					{
						sessionId: session.sessionId,
						atSeq: typeof session.lastSeq === "number" && session.lastSeq >= 0 ? session.lastSeq : undefined,
						increaseTitle: true,
					},
		...
		function EntryCard({ t, entry, ctx }) {
			...
			return jxs("div", {
				...
				children: [
					...
					prefsState.selectMode !== true ? jxs("div", { className: "kidc_actions", onClick: (e) => e.stopPropagation(), children: entry.kind === "input"
						? [
							jx("button", { type: "button", className: "kidc_iconBtn", title: t("edit"), onClick: () => prefsActions.patch({ editorVisible: true, editor: entry.id }), children: jx(_primitives.IconEditOutline16, { size: 13 }) }),
		...
		function EntryList({ t, ctx }) {
			...
			return jxs("div", { className: "kidc_list kidc_treeList", children: [
				folderGroups.map((group) => {
					...
					return jxs("div", { key: "grp-" + group.id, className: "kidc_treeGroup", children: [
						...
						expanded === true
							? jxs("div", { className: "kidc_treeItems", children: group.items.map((entry) => jx(EntryCard, { key: entry.id, entry, t, ctx })) })
							: null,
					] });
				}),
				loose.map((entry) => jx(EntryCard, { key: entry.id, entry, t, ctx })),
			] });
		}
		...
		function WindowHeader({ t, ctx, collapsed }) {
			...
			const onPointerDown = (e) => {
				...
				const baseX = win !== null && win !== undefined && typeof win.x === "number" ? win.x : 24;
				const baseY = win !== null && win !== undefined && typeof win.y === "number" ? win.y : 96;
				drag.current = { startX, startY, baseX, baseY, moved: false, id: e.pointerId };
				const onMove = (ev) => {
				...
				children: [
					...
					jxs("div", { className: "kidc_footer", children: [
						...
						jx("button", {
							...
							onClick: () => void captureSession(ctx, t),
							children: jx(_primitives.IconArchiveOutline20, { size: 13 }),
						}),
						jx("button", { type: "button", className: "kidc_primary", onClick: () => captureDraft(ctx, t, app.notify), children: jx(_primitives.IconPlusOutline16, { size: 13 }) }),
					]}),
				],
			});
		}
		...
		function measureDock() {
			try {
				...
				return { top: Math.max(0, top), bottom: vh, rootEl };
			} catch {
				return null;
			}
		}

		/** 停靠时给会话区根设置 margin 挤出（右停靠=右侧缩进，左停靠=左侧缩进）。返回恢复函数。 */
		function applyDockInset(dock, width) {
			try {
				const geom = measureDock();
				if (geom === null || geom.rootEl === null || geom.rootEl === undefined) return () => {};
				const el = geom.rootEl;
				const prev = { right: el.style.marginRight, left: el.style.marginLeft };
				// 仅右停靠：右侧缩进
				el.style.marginRight = dock === "right" ? width + "px" : "0px";
		...
		function ClipboardWindow({ t, ctx }) {
			...
			const clearDockInset = () => {
				try {
					const geom = measureDock();
					if (geom !== null && geom.rootEl !== null && geom.rootEl !== undefined) {
						geom.rootEl.style.marginRight = "0px";
						geom.rootEl.style.marginLeft = "0px";
					}
				} catch {
					/* noop */
				}
			...
			useEffect(() => {
				// 收起/关闭时解除挤出，工作区恢复；展开/打开时重新挤出
				if (dockForEffect === null || collapsedForEffect || !openForEffect) {
					clearDockInset();
					return undefined;
				}
			...
			// dock 几何缓存（仅停靠状态变化时测量；窗口 resize 时重测）。hook 必须在早退前调用。
			const dockGeomCache = useRef(null);
			const prevDockForRef = useRef(null);
			if (prevDockForRef.current !== dockForEffect) {
				prevDockForRef.current = dockForEffect;
				dockGeomCache.current = dockForEffect !== null ? measureDock() : null;
			}
			useEffect(() => {
				if (dockForEffect === null) return undefined;
				const onResize = () => {
					dockGeomCache.current = measureDock();
					prefs.actions.patch({ bump: (prefs.getSnapshot().bump ?? 0) + 1 });
				};
			...
			const vw = typeof window !== "undefined" ? window.innerWidth : 1280;
			const vh = typeof window !== "undefined" ? window.innerHeight : 720;
			const width = win !== null && win !== undefined && typeof win.width === "number" && Number.isFinite(win.width)
				? Math.max(280, Math.min(win.width, vw - 16))
				: Math.min(360, vw - 16);
			const maxHeight = Math.min(960, vh - 32);
			const height = win !== null && win !== undefined && typeof win.height === "number" && Number.isFinite(win.height)
			...
			const anchorLeft = dock === "right" ? vw - width - 8 : x;
			// 停靠：top = tab 行下沿（会话消息区顶），高度到底（窗口底）；并将会话区挤出
			// dock 几何已在上方缓存（ref）；缩放宽度不重测避免顶部跳动，窗口 resize 时重测
			const dockGeom = dock !== null ? dockGeomCache.current : null;
			const dockedTop = dock !== null && dockGeom !== null ? dockGeom.top : (dock !== null ? 0 : y);
			const dockedHeight = dock !== null && dockGeom !== null
				? Math.max(200, dockGeom.bottom - dockGeom.top)
				: (dock !== null ? vh : height);
			// 挤出会话区：hook 已在上方（open 前）无条件调用；此处仅保留几何计算
			// dock 停靠：右端固定（right 定位），左端随宽度伸缩；浮动：left+top 定位
			const style = dock === "right" && prefsState.collapsed !== true
				? {
					left: "auto",
					right: 8,
					top: dockedTop,
				...
				: {
					...
					top: prefsState.collapsed === true ? Math.max(0, Math.min(y, vh - 160)) : dockedTop,
					width: prefsState.collapsed === true ? undefined : width,
					height: prefsState.collapsed === true ? undefined : dockedHeight,
				};
			return jxs("div", {
				...
				children: [
					prefsState.collapsed === true
						...
						: jxs("div", { className: "kidc_panelWrap" + (dock !== null ? " kidc_dockedRight" : ""), style: { height: "100%" }, children: [
							...
							jx(ResizeHandle, { t, data, docked: dock !== null }),
						] }),
					jx(EditorModal, { t, ctx }),
					jx(FolderModal, { t }),
					jx(TagModal, { t }),
					jx(SessionModal, { t, ctx }),
					jx(ConfirmModal, { t }),
				],
			});
		}

		/** PS 风格收起条：贴窗口右缘竖排小标签，点击展开。 */
		function DockedPill({ t, dock, y }) {
			const { actions: prefsActions } = prefs;
			const vh = typeof window !== "undefined" ? window.innerHeight : 720;
			const dockGeom = measureDock();
			// 与展开面板同一 top 基准（tab 行下沿）；测量失败回退 y
			const pillTop = dockGeom !== null ? Math.max(0, dockGeom.top) : Math.max(0, Math.min(y, vh - 60));
			const style = {
				top: pillTop,
				left: "auto",
				right: 0,
		...
		function ResizeHandle({ t, data, docked }) {
			...
			const onPointerDown = (e) => {
				...
				dragRef.current = {
					startX: e.clientX,
					startY: e.clientY,
					baseW: data.settings?.window?.width ?? 360,
					baseH: data.settings?.window?.height ?? Math.min(560, (typeof window !== "undefined" ? window.innerHeight : 720) - 32),
				};
				const onMove = (ev) => {
					...
					const vw = typeof window !== "undefined" ? window.innerWidth : 1280;
					const vh = typeof window !== "undefined" ? window.innerHeight : 720;
					// dock 右停靠：向左侧拖 = 加宽，向右侧拖 = 减窄（右端固定）
					const delta = docked === true ? -(ev.clientX - d.startX) : (ev.clientX - d.startX);
					const nextW = Math.max(280, Math.min(d.baseW + delta, vw - 16));
					if (docked === true) {
						commitWindowSize(Math.round(nextW), undefined);
					} else {
						const nextH = Math.max(200, Math.min(d.baseH + (ev.clientY - d.startY), Math.min(960, vh - 32)));
						commitWindowSize(Math.round(nextW), Math.round(nextH));
					}
				};
				const onUp = (ev) => {
					active.current = false;
					dragRef.current = null;
					try { e.currentTarget.releasePointerCapture?.(ev.pointerId); } catch { /* noop */ }
					window.removeEventListener("pointermove", onMove);
					window.removeEventListener("pointerup", onUp);
				};
				window.addEventListener("pointermove", onMove);
			...
			return jx("div", {
				className: "kidc_resizeHandle" + (docked === true ? " kidc_resizeHandleDock" : ""),
				title: t("resizeHint"),
				onPointerDown,
				// dock 时纯竖条无图标（避免顶部弧形干扰）
				children: docked === true ? null : jx(_primitives.IconPanelLeftOutline16, { size: 12 }),
			});
		}

		/** 持久化窗口宽高到 settings.window（与 x/y/expanded 同源）。 */
		function commitWindowSize(widthValue, heightValue) {
			commitData((d) => {
				const current = d.settings.window ?? {};
				d.settings.window = {
		...
		function SettingsPanel({ t }) {
			...
			const clearAll = () => {
				...
				prefs.actions.patch({ selected: [], selectMode: false, sessionModal: null, editorVisible: false, editor: null, folderModal: false, tagModal: false });
				setArmed(false);
				app.notify("settingsCleared");
			};
			return jxs("div", {
				className: "kidc_settings",
				children: [
					jxs("section", { className: "kidc_setSection", children: [
						jx("div", { className: "kidc_setHead", children: t("settingsWindow") }),
					...
					jxs("section", { className: "kidc_setSection", children: [
						jx("div", { className: "kidc_setHead", children: t("settingsCapture") }),
						jxs("div", { className: "kidc_setRow", children: [
							jxs("div", { className: "kidc_setText", children: [
					...
					jxs("section", { className: "kidc_setSection", children: [
						jx("div", { className: "kidc_setHead", children: t("settingsData") }),
						jxs("div", { className: "kidc_setRow", children: [
							jxs("div", { className: "kidc_setText", children: [
					...
					jxs("section", { className: "kidc_setSection", children: [
						jx("div", { className: "kidc_setHead", children: t("settingsAbout") }),
						jx("div", { className: "kidc_setHint", children: t("settingsVersion", { version: "0.1.0" }) }),
					]}),
				],
			});
		}
		...
		function apply(ctx) {
			...
			ctx.slots.inject("kidai-hub.tabs", () => ctx.slots.register({
				name: "kidai-hub.tabs",
				key: "kidai-clipboard",
				order: 60,
				...
				void props;
				return jx(AppCtxProvider, { value: provide, children: jx(SettingsPanel, { t: provide.t }) });
			}));
