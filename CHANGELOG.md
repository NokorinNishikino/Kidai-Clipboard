...
### 新增 / Added
- **拖动排序 + 拖到条目上自动归类**：把条目拖到**另一个条目**上时会插到它的前面/后面（按鼠标在卡片的上半/下半决定，卡片上会出现一条高亮插入线），并**自动归入目标条目所属的文件夹** —— 排序与归类是同一套手势完成的。条目新增 `order` 字段（同容器内自定义位次），排序规则为：置顶优先 → 手动 `order` → 最近更新；host 侧 `sanitizeEntry` 已放行该字段。

### 新增 / Added
- **文件夹编辑弹窗**：文件夹树表头与「管理 → 文件夹」列表都新增编辑按钮，打开与「编辑输入」同款的弹窗 —— 可修改**名称**、从调色板里选**颜色**（含「无色」），也能直接删除；`Enter` 保存、`Esc` 关闭。设置状态存在 `prefs.folderEditor`，打开时自动回填当前值。
- **拖拽移动条目（类文件管理器）**：在列表里直接拖动条目卡片即可整理 —— 拖到**文件夹行**上移入该文件夹（悬停时目标高亮虚线框），拖到**列表空白**处移出文件夹；多选模式下自动禁用拖拽避免误触，移动后给出提示（「已移动到「X」」/「已移出文件夹。」）。实现用原生 HTML5 拖放（`draggable` + `dragstart/dragover/drop`），拖动中 `dragover` 在文件夹行上 `stopPropagation`，否则事件会冒泡到列表根、把高亮目标覆盖成"移出文件夹"。
